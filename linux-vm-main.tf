...
resource "aws_eip" "linux-eip" {
  vpc  = true
  tags = {
    Name        = "${lower(var.server_name)}-${var.server_environment}-linux-eip"
...
resource "aws_instance" "linux-server" {
  ...
  tags = {
    Name        = "${lower(var.server_name)}-${var.server_environment}-linux-server"
    Environment = var.server_environment
  }
}
...
resource "aws_security_group" "aws-linux-sg" {
  name        = "${lower(var.server_name)}-${var.server_environment}-linux-sg"
